...
# from .data import SBM_loader, geology_loader, coin_loader
...
def Laplace_dos(fname, Nmoment=50, nZ=50, npts=20, svd=False, compute_range=False, save=True):
    ...
    for i in range(len(G_times)):
        ...
        if (i in cps):
            vecs.append(
                get_dos(L.copy(), Nmoment=Nmoment, nZ=nZ, npts=npts, outname="none", compute_range=compute_range))
        else:
            vecs.append(
                get_dos(L.copy(), Nmoment=Nmoment, nZ=nZ, npts=npts, outname="none", compute_range=compute_range))
        end = timeit.default_timer()
...
def graph_to_laplace_dos(G_times, Nmoment=50, nZ=50, npts=20, svd=False, compute_range=False):
    vecs = []
    if svd:
        svd_vecs = []
