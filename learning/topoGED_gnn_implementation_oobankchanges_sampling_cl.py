...
class Runner(object):
    def __init__(self, args):
        ...
        # Per-vector forecaster choice; falls back to --rnn_type for both
        self.toper_method = args.toper_method or self.rnn_type
        self.probs_method = args.probs_method or self.rnn_type
        
        # Set up Evaluator
        self.evaluator = Evaluator() if Evaluator is not None else None
        ...
        self.saved_input = os.path.join(_learning, f'datasets/cached/{self.dataset}/saved_data_gnn_{self.model_type}{self.feature_type}_lr{encoder_config["training"]["lr"]}_{days_back_val}back_oobankchanges_sampling_tmp')
        self.saved_samples = os.path.join(self.saved_input, 'saved_samples.pkl')
        if self.toper_method == self.probs_method:
            rnn_tag = f'_rnn{self.toper_method}' if self.toper_method else ''
        else:
            rnn_tag = f'_toper{self.toper_method}_probs{self.probs_method}'
        ...
        self.saved_graph_dir = os.path.join(_learning, f'output/constructed_graphs/{self.dataset}_{self.common_suffix}_edgebank_{self.edgebank_style}_VectorType{encoder_config["use_test_style"] if encoder_config["use_predicted_vals"] else "TrueVals"}')
        ...
        self.probabilities, self.graph_descriptions, self.thresholds, self.target_graphs = load_data(self.dataset, encoder_config["encoder_model"]["addOnFeature"],
                                                                                                     encoder_config["decoder_model"]["encode_links"], encoder_config["encoder_model"]["nodeEmbeddingType"], days_back_val, use_predicted=False, num_buckets=encoder_config["num_toper_buckets"], use_test_style=None)
        
        # Modify the graph ids to 1,2,3,...
        self.target_graphs, _ = modifyGraphIds(self.target_graphs, self.thresholds, 10000)

        # Replace prob and toper vectors with predicted vectors if requested.
        # Methods can differ per vector type (e.g. --toper_method LSTM --probs_method RNN);
        # predictions are loaded from the pkls written by generate_toper_cl.py / generate_probs_cl.py.
        ...
        def _load_pred_vectors(kind, method):
            ...
            if not os.path.exists(path):
                raise FileNotFoundError(
                    f'{path} not found — run learning/generate_{kind}_cl.py with "{name}" in its methods list first')
            with open(path, 'rb') as _f:
                return _pkl.load(_f)

        if self.probs_method is not None:
            self.probabilities = _load_pred_vectors('probs', self.probs_method)
            print(f'[INFO] Replaced probabilities with {self.probs_method} predictions')
        if self.toper_method is not None:
            self.graph_descriptions = _load_pred_vectors('toper', self.toper_method)
            print(f'[INFO] Replaced graph_descriptions with {self.toper_method} predictions')
        ...
        for ablation_mode in ablation_modes:
            for toper_len in sensitivity_params:
                ...
                if ablation_mode > 0:
                    ...
                    self.target_graphs, _ = modifyGraphIds(self.target_graphs, self.thresholds, 10000)
                    self.graph_descriptions = [[(lst[i], lst[i+1]) for i in range(0, len(lst), 2)] for lst in self.graph_descriptions]
                    self.saved_graph_dir = f'data/output/sensitivity_analysis/constructed_graphs/{self.dataset}_{self.common_suffix}_edgebank_{self.edgebank_style}_VectorType{encoder_config["use_test_style"] if encoder_config["use_predicted_vals"] else "TrueVals"}_len{toper_len}'
                    os.makedirs(self.saved_graph_dir, exist_ok=True)
