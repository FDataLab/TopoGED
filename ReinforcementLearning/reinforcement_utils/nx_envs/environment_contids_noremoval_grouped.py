...
class GraphReconstructionNxContidsNoRemovalGrouped(gym.Env):
    ...
    def __init__(self, feature_vectors, filtration_thresholds, probabilities, target_graphs, max_steps_per_graph = 1250, expert_trajectories=None, embed_graph=False, all_graphs=None):
        super(GraphReconstructionNxContidsNoRemovalGrouped, self).__init__()
        ...
        self.pred_final_storage = []
        self.target_final_storage = []
    ...
    def step(self, action):
        ...
        edge_reward = 75  # Lower reward for old edges (since they're more frequent)
        ...
        if action_type == 0:  
            ...
            if (node1 in self.graph.nodes(data=False) and node2 in self.graph.nodes(data=False)) and not self.graph.has_edge(node1, node2):
                if self.graph.nodes[node1]['feat']['currDegree'] < self.graph.nodes[node1]['feat']['maxDegree'] and self.graph.nodes[node2]['feat']['currDegree'] < self.graph.nodes[node2]['feat']['maxDegree'] and self.edges_to_place > 0:
                    placed_edge = False
                    
                    # Edge OO
                    if self.resources[2] > 0 and node1 < self.old_nodes_end and node2 < self.old_nodes_end and node2 in self.edge_bank.get(node1, []):
                        placed_edge = True
                        self.resources[2] -= 1
                        
                    # Edge NN    
                    elif self.resources[3] > 0 and node1 >= self.old_nodes_end and node2 >= self.old_nodes_end:
                        placed_edge = True
                        self.resources[3] -= 1
                        
                    # Edge ON    
                    elif self.resources[4] > 0 and node1 < self.old_nodes_end and node2 >= self.old_nodes_end:
                        placed_edge = True
                        self.resources[4] -= 1
                    ...
                    # Edge OON   
                    elif self.resources[5] > 0 and node1 < self.old_nodes_end and node2 < self.old_nodes_end and node2 not in self.edge_bank.get(node1, []):
                        placed_edge = True
                        self.resources[5] -= 1
                    ...
                    if placed_edge:
                        ...
                        reward = edge_reward + self.compute_reward_single_edge(node1, node2)
                        print('add an edge success')
    ...
    def reset(self, seed=None, options=None):
        ...
        if(self.curr_graph >= len(self.feature_vectors)):
            self.done = True  # Modify to work on info
                ...
                for edge in subgraph.edges(data=False):
                    if edge in existing_edges:
                        ...
                        if(edge in old_edges):
                            decisions.append((decision_map['add_edge'], node1, node2))
                            ...
                            decisions.append((decision_map['add_edge'], node2, node1))
