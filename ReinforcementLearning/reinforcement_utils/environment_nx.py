...
class TemporalGraphEnv(gym.Env):
    def __init__(self, max_nodes=100, max_edges=200):
        super(TemporalGraphEnv, self).__init__()

        # Graph tracking
        self.G = nx.Graph()  # Current graph
        self.G_prev = nx.Graph()  # Previous graph
        self.seen_nodes = set()  # Nodes from previous graphs
        self.seen_edges = set()  # Edges from previous graphs
        self.node_counter = 0  # Tracks new node IDs

        # Define action space (discrete)
        self.action_space = spaces.Discrete(8)

        # Define observation space (adjacency matrix + node degree vector)
        self.observation_space = spaces.Dict({
            "adjacency_matrix": spaces.Box(low=0, high=1, shape=(max_nodes, max_nodes), dtype=np.float32),
            "resources": spaces.Box(low=0, high=max_edges, shape=(max_nodes,), dtype=np.float32)
        })

    def reset(self):
        """Reset the environment at the start of an episode."""
        self.G_prev = self.G.copy()  # Store old graph
        self.G.clear()  # Reset graph
        self.node_counter = 0  # Reset ID counter
        return self.get_observation()

    def step(self, action):
        """Take an action and update the graph accordingly."""
        reward = 0  # Default reward

        if action == 0:  # Use a previously seen edge
            reward += self.use_old_edge()
        elif action == 1:  # Make an edge between two new nodes
            reward += self.add_new_edge()
        elif action == 2:  # Make an edge between one new and one old node
            reward += self.add_mixed_edge()
        elif action == 3:  # Make an edge between two old nodes that did not previously have an edge
            reward += self.add_old_edge()
        elif action == 4:  # Remove an edge
            reward += self.remove_edge()
        elif action == 5:  # Activate old node
            reward += self.activate_old_node()
        elif action == 6:  # Activate new node
            reward += self.activate_new_node()
        elif action == 7:  # Remove a node
            reward += self.remove_node()

        return self.get_observation(), reward, False, {}  # No termination condition yet

    def use_old_edge(self):
        """Use an old edge from the previous graph."""
        if self.seen_edges:
            edge = random.choice(list(self.seen_edges))
            self.G.add_edge(*edge)
            return 1  # Reward for reusing structure
        return -1  # Penalty for no old edges available

    def add_new_edge(self):
        """Add an edge between two new nodes."""
        n1, n2 = self.node_counter, self.node_counter + 1
        self.G.add_edge(n1, n2)
        self.node_counter += 2  # Increment counter
        return 1  # Reward for adding new structure

    def add_mixed_edge(self):
        """Add an edge between a new and an old node."""
        if self.seen_nodes:
            old_node = random.choice(list(self.seen_nodes))
            new_node = self.node_counter
            self.G.add_edge(old_node, new_node)
            self.node_counter += 1
            return 1  # Reward for linking past and future
        return -1  # No old nodes available

    def add_old_edge(self):
        """Add an edge between two old nodes that didn't previously connect."""
        old_nodes = list(self.seen_nodes)
        if len(old_nodes) > 1:
            n1, n2 = random.sample(old_nodes, 2)
            if not self.G_prev.has_edge(n1, n2):  # Ensure no prior connection
                self.G.add_edge(n1, n2)
                return 1
        return -1  # No valid old nodes
    ...
    def remove_edge(self):
        ...
        if self.G.edges:
            ...
            return 1
        return -1  # No edges to remove

    def activate_old_node(self):
        """Reactivate an old node."""
        if self.seen_nodes:
            node = random.choice(list(self.seen_nodes))
            self.G.add_node(node)
            return 1
        return -1

    def activate_new_node(self):
        """Activate a new node."""
    ...
    def remove_node(self):
        ...
        if self.G.nodes:
            ...
            return 1
        return -1  # No nodes to remove

    def get_observation(self):
        """Return the observation space (adjacency matrix + node degrees)."""
        adj_matrix = nx.to_numpy_array(self.G, dtype=np.float32)
        node_degrees = np.array([self.G.degree(n) for n in self.G.nodes], dtype=np.float32)
        return {
