...
csv_file_path = os.path.abspath('data/output/results/BinaryTesting/data/embedding_testing.csv')
...
if not os.path.isfile(csv_file_path):
    pd.DataFrame(columns=['run_id', 'dataset', 'activation', 'seed', 'hidden_dim_1', 'hidden_dim_2', 'mlp_dim', 'learning_rate', 'num_layers_LSTM', 'num_layers_GRU', 'trained_epochs', 'train_loss', 'train_aucroc', 'valid_loss', 'valid_aucroc']).to_csv(csv_file_path, index=False)
...
# Grid search params
datasets = ['networkcoindash', 'networkcindicator', 'networkaion', 'networkadex',  'networkaeternity',  'networkaragon', 'networkbancor', 'networkcentra',
             'mathoverflow', 'CollegeMsg', 'Reddit_B']
num_layers = [3, 2]
dropouts = [0, 0.2, 0.35]
                        ...
                        for mlp_dim in mlp_dims:
                            for lr_val in learning_rates:
                                for l2_val in l2_regularizations:
                                    # Initialize wandb
                                    run = wandb.init(project="embedding_activations_gs", config={
                                        'dataset': dataset,
