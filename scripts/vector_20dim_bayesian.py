...
# Constants
csv_file_path = os.path.abspath('data/output/results/RegressionTesting/data/embedding_testing_bayesian_regression_20dim_deltas.csv')
...
os.makedirs(os.path.dirname('output/cached_model/RegressionTesting/bayesianSave/model_selection_regression_20dim_deltas.db'), exist_ok=True)
seed = 42  # Can change
...
def train_and_eval(dataset, activations, window_size, norm, num_layer, dropout, hidden_1, hidden_2, lr_val, l2_val, batch_size, combo, counter, seed, csv_file_path):
    ...
    # Initialize wandb
    run = wandb.init(
        project="bayesian_testing_regression_20dim", 
        name = run_name, 
        config={
        'dataset': dataset,
...
def main():
    ...
    #optuna.delete_study(study_name="model_selection_20dim_deltas", storage=STORAGE)
    study = optuna.create_study(study_name="model_selection_20dim_deltas", storage=STORAGE, direction="minimize", load_if_exists=True)
