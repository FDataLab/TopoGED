...
for dataset in datasets:
    ...
    for activation, activation_name in zip(activations, activation_names):
        ...
        data, labels = my_loader.load_data(dataset, activation_name)
                            ...
                            for mlp_dim in mlp_dims:
                                for lr_val in learning_rates:
                                    for l2_val in l2_regularizations:
                                        model = LSTMGRU_MLP(input_dim, output_dim, hidden_dim_1=hidden_1, hidden_dim_2=hidden_2, mlp_dim=mlp_dim, dropout=dropout, num_layers_LSTM=num_layer, num_layers_GRU=num_layer).to(device)
                                        ...
                                        for epoch in range(num_epochs):
                                            model.train()
                                            train_loss, train_aucroc, train_aucpr, train_accuracy = model.train_model_binary(model, train_loader, optimizer, criterion, device=device)
                                            valid_loss, valid_aucroc, valid_aucpr, valid_accuracy = model.test_model_binary(model, valid_loader, criterion, y_val, device=device)
