...
csv_file_path = os.path.abspath('data/output/results/ProbabilityTesting/data/probability_testing_bayesian_individual_regression_multi.csv')
...
def train_and_eval(dataset, window_size, num_layer, dropout, hidden_1, lr_val, l2_val, batch_size, combo, counter, seed, csv_file_path):
    ...
    # Initialize wandb
    run = wandb.init(
        project="bayesian_testing_probabilities_multi", 
        name = run_name, 
        config={
        'dataset': dataset,
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss <= curr_batch_best_loss:
            ...
            if(dataset == 'cosine'):
                ...
                tmp_df.to_csv('data/output/results/RegressionTesting/exampleEmbeddings/cosine_real_ex_multi.csv')
                tmp_df = pd.DataFrame(predicted_embeddings, columns = columns)
                tmp_df.to_csv('data/output/results/RegressionTesting/exampleEmbeddings/cosine_pred_ex_multi.csv')
...
def main():
    ...
    wandb.init(project="Probabilities", name="probabilities_bayesian_testing_individual_dataset_4060_multi")    

    # _4060 SIGNIFIES THAT WE ARE USING 40% TRAIN AND 60% VAL IN OUR BAYESIAN SCORING
    STORAGE = f"sqlite:///./output/cached_model/ProbabilityTesting/bayesianSave/model_selection_4060_multi.db"  # Where we save the study
    os.makedirs(os.path.dirname(f'output/cached_model/ProbabilityTesting/bayesianSave/model_selection_4060_multi.db'), exist_ok=True)    
    study_name = f"probabilities_model_selection_4060_multi"  # Unique study name for each dataset
