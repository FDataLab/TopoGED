...
def train_and_eval(dataset, prob_type, prob_type_idx, window_size, num_layer, dropout, hidden_1, lr_val, l2_val, batch_size, combo, counter, seed, csv_file_path):
    # Setup
    my_loader = Loader()
    my_utils = Utils()
...
def objective(trial, prob_type):
    ...
    csv_file_path = os.path.abspath(f'data/output/results/ProbabilityTesting/data/probability_testing_bayesian_individual_regression_multi_{prob_type}.csv')
...
def main():
    ...
    for prob_type in prob_types.keys():
        wandb.init(project="Probabilities", name=f"probabilities_bayesian_testing_individual_dataset_4060_multi_type_{prob_type}")    
        ...
        study_name = f"probabilities_model_selection_4060_multi_{prob_type}"  # Unique study name for each dataset
        ...
        study.optimize(partial(objective, prob_type=prob_type), n_trials=500)
