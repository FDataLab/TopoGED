...
def normalize_vector_by_groups(vec, tol=1e-8):
    ...
    group1 = vec[0:2]
    sum1 = np.sum(group1)
    vec[0:2] = group1 / sum1 if sum1 > 0 else 0
    ...
    group2 = vec[2:6]
    sum2 = np.sum(group2)
    vec[2:6] = group2 / sum2 if sum2 > 0 else 0
...
def train_and_eval_delta(dataset, window_size, num_back, norm, num_layer, dropout, hidden_1, hidden_2, lr_val, l2_val, batch_size, combo, counter, seed):
    ...
    if norm:
        embeddings = probabilities_df.apply(
            lambda row: normalize_vector_by_groups(row.values),
            axis=1
        )
...
if __name__ == "__main__":
    ...
    for _, row in best_trials.iterrows():
        ...
        norm_pred_embeddings = np.array([normalize_vector_by_groups(row) for row in best_pred_embeddings])
        norm_real_embeddings = np.array([normalize_vector_by_groups(row) for row in best_real_embeddings])
        ...
        for i, name in enumerate(col_names):
            plt.figure()
            plt.scatter(norm_real_embeddings[:, i], norm_pred_embeddings[:, i], alpha=0.7)
            plt.xlabel(f"Normalized Real {name}")
            plt.ylabel(f"Normalized Pred {name}")
            plt.title(f"Normalized Pred vs Real: {name}")
            plt.plot([0, 1], [0, 1], color="red", linestyle="--")  # 45-degree reference line
            plt.xlim(0, 1)
            plt.ylim(0, 1)
            plt.tight_layout()
            plot_path = os.path.join(save_dir, f"scatter/Norm_Pred_vs_Real_{name}.png")
            plt.savefig(plot_path)
            plt.close()
            print(f"Saved normalized plot for {name} to {plot_path}")
            
            # --- Line plot ---
            plt.figure()
            plt.plot(norm_real_embeddings[:, i], label=f"Real {name}", color="blue")
            plt.plot(norm_pred_embeddings[:, i], label=f"Pred {name}", color="orange")
            plt.xlabel("Sample index")
            plt.ylabel("Probability")
            plt.title(f"Pred vs Real (line): {name}")
            plt.legend()
            plt.ylim(0, 1)
            plt.tight_layout()
            ...
            plt.close()
            print(f"Saved line plot for {name} to {line_path}")
        ...
        for i, name in enumerate(col_names):
            plt.figure()
            plt.scatter(best_real_embeddings[:, i], best_pred_embeddings[:, i], alpha=0.7)
            plt.xlabel(f"Real {name}")
            plt.ylabel(f"Pred {name}")
            plt.title(f"Pred vs Real: {name}")
            plt.plot([best_real_embeddings[:, i].min(), best_real_embeddings[:, i].max()],
                    [best_real_embeddings[:, i].min(), best_real_embeddings[:, i].max()],
                    color="red", linestyle="--")  # 45-degree reference
            plt.tight_layout()
            plot_path = os.path.join(save_dir, f"scatter/Pred_vs_Real_{name}.png")
            plt.savefig(plot_path)
            plt.close()
            ...
            # --- Line plot ---
            plt.figure()
            plt.plot(best_real_embeddings[:, i], label=f"Real {name}", color="blue")
            plt.plot(best_pred_embeddings[:, i], label=f"Pred {name}", color="orange")
            plt.xlabel("Sample index")
            plt.ylabel("Probability")
            plt.title(f"Pred vs Real (line): {name}")
            plt.legend()
