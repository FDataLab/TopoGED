...
# Constants
csv_file_path = os.path.abspath('data/output/results/BinaryTesting/data/embedding_testing_bayesian_2.csv')
model_dir = os.path.abspath('data/output/cached_model/BinaryTesting/EmbeddingTesting')
STORAGE = "sqlite:///./output/cached_model/BinaryTesting/bayesianSave/model_selection_2.db"  # Where we save the study
os.makedirs(os.path.dirname('output/cached_model/BinaryTesting/bayesianSave/model_selection_2.db'), exist_ok=True)
seed = 42  # Can change
...
def main():
    ...
    wandb.init(project="Binary", name="bayesian_testing_2")
    #optuna.delete_study(study_name="model_selection", storage=STORAGE)
    study = optuna.create_study(study_name="model_selection", storage=STORAGE, direction="maximize", load_if_exists=True)
