...
def get_prediction_edges(undirected_edge_index_list):
    ...
    for undirected_edge in undirected_edge_index_list:
        ...
        neg_edges = negative_sampling(undirected_edge, num_neg_samples=pos_edges.size(1))  # @TODO: different negative sampling should be employed here!
        neg_edges_list.append(neg_edges)
    return pos_edges_list, neg_edges_list


def get_prediction_edges_modified(undirected_edge_index_list):
    """
    similar to 'get_prediction_edges' function but only for one set of edges: either positive or negative
    """
    edges_list = []
    for undirected_edge in undirected_edge_index_list:
        edges = to_one_directed_edge(undirected_edge)
...
def get_new_prediction_edges_modified(undirected_edge_index_list, num_nodes):
    """
    similar to the 'get_new_prediction_edges' function only for one set: either positive or negative edges
    """
    edges_list = [torch.zeros((2, 100))]  # ignore the first pos edges
    ...
    for i in range(1, len(undirected_edge_index_list)):
        ...
        perm = np.setdiff1d(edges_perm, np.intersect1d(edges_perm, last_edges_perm))  # new edges: edge-edge^last_edge
        edges = np.vstack(np.divmod(perm, num_nodes)).transpose().astype(np.long)  # convert perm to indices
        edges = torch.from_numpy(edges).transpose(1, 0)
