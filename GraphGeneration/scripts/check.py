...
def compute_node2vec_embeddings2(G: nx.DiGraph):
    """
    Use Node2Vec to embed nodes in the constructed graph. Appends node features onto the end since Node2Vec does not account for features
    One of two available methods
...
def are_funcs_identical_bytecode(f1, f2):
    return f1.__code__.co_code == f2.__code__.co_code

print(are_funcs_identical_bytecode(compute_node2vec_embeddings, compute_node2vec_embeddings2))