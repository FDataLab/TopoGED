...
# Import all embedding methods in case we want them
...
parser.add_argument("--dataset", type=str, required=True, choices=['CollegeMsg', 'mathoverflow', 'networkadex', 'networkaeternity', 'networkaion', 'networkaragon', 'networkbancor', 'networkcentra', 'networkcindicator', 'networkcoindash', 'networkdgd', 'networkiconomi', 'Reddit_B'], help='The dataset to use')
parser.add_argument("--oldDegree", type=str, required=True, choices=['True', 'False'], help='Whether or not you want to use the nodes previously used degree') 
...
def build_accumulating_filtration_sequence_with_edgebank(embedding, p_old_nodes, p_new_nodes, E_oo, E_nn, E_on, E_oon, graph_num, edgebank=None, existing_nodes=None, seed=42):
    ...
    Specifically, this version takes into account node degree to guide construction based on TopER vectors and randomly places edges if there is space
    ...
    prev_count = 0
    for i, (degree, current_count) in enumerate(degree_dict.items()):
        if i == 0:
            delta = current_count
        else:
            delta = current_count - prev_count

        degree_assignment.extend([degree] * max(delta, 0))
        prev_count = current_count
        
    random.shuffle(degree_assignment)
    
    # If we want to reuse the old degree
    if args.oldDegree == True:
        for node in old_nodes:
            ...
            max_degrees[node] = assigned_degree
            curr_degrees[node] = 0
        ...
        for i, node in enumerate(all_nodes):
            max_degrees[node] = degree_assignment[i]
            curr_degrees[node] = 0
                ...
                if u in edgebank:
                    for v in edgebank[u]:
                        if v in dst_list and u != v and v in edgebank.get(u, []) and (u, v) not in edges:
                            if((curr_degrees[u] < max_degrees[u]) and (curr_degrees[v] < max_degrees[v])):
                                sampled.add((u, v))
...
# Construct csvs
run_number = 1
structure_pred_file_path = f'GraphGeneration/output/results/structure/{dataset}/contids_degree_oldDegree{args.oldDegree}/structure_pred.csv'
structure_true_file_path = f'GraphGeneration/output/results/structure/{dataset}/contids_degree_oldDegree{args.oldDegree}/structure_true.csv'
structure_diff_file_path = f'GraphGeneration/output/results/structure/{dataset}/contids_degree_oldDegree{args.oldDegree}/structure_diff.csv'
kernel_pred_file_path = f'GraphGeneration/output/results/kernel/{dataset}/contids_degree_oldDegree{args.oldDegree}/kernel_pred.csv'
kernel_true_file_path = f'GraphGeneration/output/results/kernel/{dataset}/contids_degree_oldDegree{args.oldDegree}/kernel_true.csv'
edge_file_path = f'GraphGeneration/output/results/structure/{dataset}/contids_degree_oldDegree{args.oldDegree}/edge_analysis.csv'
topER_file_path = f'GraphGeneration/output/results/topER/{dataset}/contids_degree_oldDegree{args.oldDegree}/toper_diff.csv'
animation_path = f'GraphGeneration/output/results/animations/{dataset}/contids_degree_oldDegree{args.oldDegree}/pred_vs_true.mp4'
...
# Empty content (no columns since the number of subgraphs may change)
pd.DataFrame().to_csv(kernel_pred_file_path, index=False)
