...
def run_trial(trial_idx, dataset, target_graphs, graph_descriptions, probabilities, 
              starting_graph, num_back, param_range=(0.0, 5.0)):
    """
    Executes a single simulation trial with random continuous parameters.
    """
    # 1. Sample continuous parameters
    decay_factor = random.uniform(*param_range)
    alpha = random.uniform(*param_range)
    beta = random.uniform(*param_range)
    
    preds = []
    trues = []
    
    # 2. Run simulation across snapshots
    for i in range(starting_graph, len(probabilities)):
        v_total = int(graph_descriptions[i][-1][0])
        # Calculate target old node count
        target_count = int(round(probabilities[i][0] * v_total))
        
        # Call your probability logic
        sampled_old_nodes = compute_reappearance_probabilities_new(
            target_graphs[max(i - num_back, 0):i], 
            target_count, 
            decay_factor, alpha, beta
        )
        
        preds.append(set(sampled_old_nodes))
        trues.append(set(target_graphs[i][-1].nodes()))
    
    # 3. Vectorized Metric Calculation
    precisions, recalls, f1s = [], [], []
    for p_set, t_set in zip(preds, trues):
        tp = len(p_set & t_set)
        precision = tp / len(p_set) if len(p_set) > 0 else 0
        recall = tp / len(t_set) if len(t_set) > 0 else 0
...
if __name__ == "__main__":
    my_loader = Loader()
    num_back = 5
    num_trials_per_dataset = 100 # Adjust based on time availability
    starting_graph = 2
    
    datasets = [
        'networkadex', 'networkaion', 'networkbancor', 'networkcentra', 
        'networkcoindash', 'mathoverflow', 'Reddit_B', 'networkaragon', 
        'networkaeternity', 'networkiconomi', 'CollegeMsg', 
        'networkcindicator', 'networkdgd', 'tgbl-wiki'
    ]

    for dataset in datasets:
        print(f"--- Optimizing Old Nodes for: {dataset} ---")
        
        # Load Data
        probabilities_df = my_loader.load_data(type='probabilities', dataset=dataset, num_back=num_back)
        probabilities = probabilities_df.values.tolist()
        thresholds = my_loader.load_data(dataset, activation='Degree', type='thresholds')
        graph_descriptions, _ = my_loader.load_data(dataset, activation='Degree', type='features')
        graph_descriptions = [[(lst[i], lst[i+1]) for i in range(0, len(lst), 2)] for lst in graph_descriptions]
        target_graphs = my_loader.load_data(dataset, activation='Degree', type='subgraphs')
        target_graphs, _ = modifyGraphIds(target_graphs, thresholds, num_back)

        res_path = f"GraphGeneration/output/results/old_node_optimization/{dataset}_continuous_search.csv"
        os.makedirs(os.path.dirname(res_path), exist_ok=True)

        # Use ProcessPoolExecutor for parallel search
        # Note: If memory-bound, reduce max_workers or use a standard loop
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
            futures = [
                executor.submit(
                    run_trial, t, dataset, target_graphs, graph_descriptions, 
                    probabilities, starting_graph, num_back
                ) for t in range(num_trials_per_dataset)
            ]
            ...
            for f in futures:
                ...
                # Print progress
                print(f"Trial Done | F1: {result['mean_f1']:.4f} (Decay: {result['decay_factor']})")

        # Save all trials for this dataset
        df_results = pd.DataFrame(trial_results)
        df_results.to_csv(res_path, index=False)
        
        # Log the best found parameters to console
        best_idx = df_results['mean_f1'].idxmax()
        best = df_results.loc[best_idx]
        print(f"BEST FOUND: F1={best['mean_f1']:.4f} with Decay={best['decay_factor']}, Alpha={best['alpha']}, Beta={best['beta']}")