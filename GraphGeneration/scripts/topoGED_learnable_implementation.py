...
from GraphGeneration.encoders.TemporalNodeEmbedder import TemporalNodeEmbedder
...
class Runner(object):
    def __init__(self):      
        ...
        self.saved_input = os.path.abspath(f'data/input/cached/{encoder_config["dataset"]}/saved_data_learnable')
        ...
        self.embedder = TemporalNodeEmbedder(embedding_dim=encoder_config["encoder_model"]["node2vec_setup"]["node2vec_dimensions"], device=self.device, init_type=encoder_config["learnable_embedder"]["setup"]["init_type"])  # A learnable embedder I am trying now
    ...
    def run(self):        
        ...
        # Embedder is trained by now add the starting embeddings
        for i in range(self.starting_graph):
            self.node_embedding_history.append(self.embedder.get_embeddings(self.target_graphs[i][-1].nodes(), i))
