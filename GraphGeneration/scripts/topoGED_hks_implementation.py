...
from compute_embedding import compute_node2vec_embeddings, group_node2vec_embeddings, compute_hks_embeddings
...
class Runner(object):
    def __init__(self):      
        ...
        self.saved_input = os.path.abspath(f'data/input/cached/{encoder_config["dataset"]}/saved_data_hks_{self.use_ma}')
        self.saved_samples = os.path.join(self.saved_input, 'saved_samples.pkl')
        self.common_suffix = f'topoGED_embedding{encoder_config["encoder_model"]["addOnFeature"]}_mlpEncoding{encoder_config["decoder_model"]["encode_links"]}_embeddingType{encoder_config["encoder_model"]["nodeEmbeddingType"]}_hks_{self.use_ma}'
        ...
        # Exclusive to this iteration of topoGED (with hks)
        self.t_vals = np.linspace(0.01, 10, encoder_config["encoder_model"]["node2vec_setup"]["node2vec_dimensions"])  # Each node is half the input dim since using 'concat'
        ...
        self.node_embedding_history = [compute_hks_embeddings(self.target_graphs[i][-1], self.t_vals, device=self.device, add_degree=self.add_degree, add_time=self.add_time) for i in range(0, self.starting_graph)]  # Store the history of node embeddings for Node2Vec
    ...
    def create_samples(self, graphs, days_back, all_edgebanks, is_directed=False):
        ...
        for i, graph in enumerate(graphs):
            ...
            if i < self.starting_graph:
                all_embeddings.append(compute_hks_embeddings(graph, self.t_vals, device=self.device, add_degree=self.add_degree, add_time=self.add_time))  # Store the embedding of this graph's nodes for later use
                continue 
            ...
            if constructing_graph.number_of_edges() <= 0:
                ...
                continue
            curr_embeddings = compute_hks_embeddings(constructing_graph, self.t_vals, device=self.device, add_degree=self.add_degree, add_time=self.add_time)  # Get the current embeddings (handles empty nodes)
    ...
    def build_accumulating_filtration_sequence_with_edgebank(self, current_target_snapshot):
        ...
        curr_embeddings = compute_hks_embeddings(constructing_graph, self.t_vals, device=self.device, add_degree=self.add_degree, add_time=self.add_time)  # Re-embed the graph to get new node embeddings
    ...
    def run(self):        
        ...
        for i in range(self.starting_graph, len(self.probabilities)): 
            ...
            self.node_embedding_history.append(compute_hks_embeddings(self.target_graphs[i][-1], self.t_vals, device=self.device, add_degree=self.add_degree, add_time=self.add_time))
