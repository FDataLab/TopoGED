...
class Runner(object):
    ...
    def sample_old_nodes(self, prev_graphs):
        ...
        # --- NEW SAFETY LOGIC ---
        # Ensure we don't try to sample more nodes than exist in our history
        pop_size = len(node_ids)
        sample_size = self.current_target_count_old_nodes
        
        if sample_size > pop_size:
            print(f"WARNING: Predicted old nodes ({sample_size}) exceeds population ({pop_size}). Capping to population size.")
            sample_size = pop_size
        # ------------------------

        sampled_old_nodes = list(np.random.choice(
            node_ids, 
            size=sample_size, 
            replace=False, 
    ...
    def create_samples(self, graphs, days_back, all_edgebanks, is_directed=False):
        ...
        for i, graph in enumerate(graphs):
            ...
            # union() is faster than multiple add() calls
    ...
    def run_validation(self, batch_size, epoch, samples, snapshot_num):
        ...
        with torch.no_grad():  
            for i in range(n_graphs):
                ...
                # OPTIMIZATION: Use pre-loaded features and standard device handling
                ...
                # CRITICAL: Generate embeddings ONCE for all heads in this snapshot
                ...
                for et in active_edge_types:
                    ...
                    # Vectorized gather from the single embedding pass
        ...
        for epoch in range(encoder_config["training"]["epochs"]):
            ...
            for g in range(n_graphs):
                ...
                for et in active_edge_types:
                    ...
                    # Parallel Head Execution: gather embeddings in one indexing operation
                    preds = self.link_prediction_decoder(embeddings[u_t], embeddings[v_t], edge_type=et).view(-1)
                    
                    loss = criterion(preds, y_t.view(-1))
                    snapshot_loss += loss # Accumulate loss across all 3 heads
                    
                    train_results[et]['loss'].append(loss.item())
                    train_results[et]['auc'].append(auroc(preds.detach(), y_t.long(), task="binary").item() if y_t.unique().numel() > 1 else 0.5)
            ...
            # 1. Calculate the overall average for early stopping
            avg_val_auc = np.mean([val_res[et]['auc'] for et in active_edge_types])
            
            # 2. Display Detailed Breakdown (Loss + AUC)
            print(f"\n{'='*20} Epoch {epoch+1} {'='*20}")
            print(f"{'Edge Type':<12} | {'Tr. Loss':<10} | {'Val Loss':<10} | {'Tr. AUC':<8} | {'Val AUC':<8}")
            print("-" * 65)

            for et in active_edge_types:
                # Training metrics (averaged over snapshots)
                t_loss = np.mean(train_results[et]['loss'])
                t_auc = np.mean(train_results[et]['auc'])
                
                # Validation metrics
                v_loss = val_res[et]['loss']
                v_auc = val_res[et]['auc']
                
                print(f"{et:<12} | {t_loss:<10.4f} | {v_loss:<10.4f} | {t_auc:<8.4f} | {v_auc:<8.4f}")
            print(f">> Aggregate Val AUC: {avg_val_auc:.4f}")
    ...
    def run(self):        
        ...
        if self.mode == "ablation":
            ...
            days_backs = [encoder_config["days_back"]]  # Just use the default days back for this analysis
            toper_lens = [encoder_config["num_toper_buckets"]]  # Just use the default number of buckets for this analysis
        elif self.mode == "sensitivity_analysis_len":
            print('PERFORMING SENSITIVITY ANALYSIS ON NUMBER OF BUCKETS')
            ablation_modes = [0]
            toper_lens = [5, 10, 20, 50]
            days_backs = [encoder_config["days_back"]]  # Just use the default days back for this analysis
        elif self.mode == "sensitivity_analysis_days":
            print('PERFORMING SENSITIVITY ANALYSIS ON DAYS BACK')
            days_backs = [1, 3, 5, 7, 14, 30]
            ablation_modes = [0]
            toper_lens = [encoder_config["num_toper_buckets"]]  # Just use the default number of buckets for this analysis
        else:
            ablation_modes = [0]
            toper_lens = [encoder_config["num_toper_buckets"]]
            days_backs = [encoder_config["days_back"]]
        ...
        for ablation_mode in ablation_modes:
            for toper_len in toper_lens:
                for days_back in days_backs:
                    is_default = (ablation_mode == 0 and toper_len == 10 and days_back == 5)
                    if is_default and self.mode != "default":
                        print(f"Skipping redundant Default Run (ablation=0, len=10, days=5) during {self.mode}")
                        continue
                    print(f"Starting run with mode: {self.mode}, ablation_mode: {ablation_mode}, toper_len: {toper_len}, days_back: {days_back}")
                    # Variable setup 
                    self.days_back = days_back
                    self.H = None
                    # Re-build edgebank to respect the new window size
                    self.all_edgebanks = build_edgebanks_from_start(self.target_graphs, self.is_directed, self.days_back)

                    # Reset Node ID counter to the end of the training set
                    ...
                    if ablation_mode == 0 and self.mode == "ablation":
                        print('Not reconstructing base graphs in ablation')
                        continue
                    if ablation_mode > 0:
                        # Change it since that makes more sense now
                        self.saved_graph_dir = f'data/output/ablation/constructed_graphs/{encoder_config["dataset"]}_{self.common_suffix}_edgebank_{self.edgebank_style}_VectorType{encoder_config["use_test_style"] if encoder_config["use_predicted_vals"] else "TrueVals"}_ablation{ablation_mode}'
                        ...
                        if ablation_mode < 9:
                            ...
                            self.old_vars = {
                                'graph_descriptions': self.graph_descriptions,
                                'thresholds': self.thresholds,
                                'target_graphs': self.target_graphs,
                        ...
                        self.target_graphs, _ = modifyGraphIds(self.target_graphs, self.thresholds, 10000)
                        self.graph_descriptions = [[(lst[i], lst[i+1]) for i in range(0, len(lst), 2)] for lst in self.graph_descriptions]
                        self.saved_graph_dir = f'data/output/sensitivity_analysis/toper_lens/constructed_graphs/{self.dataset}_{self.common_suffix}_edgebank_{self.edgebank_style}_VectorType{encoder_config["use_test_style"] if encoder_config["use_predicted_vals"] else "TrueVals"}_len{toper_len}_days{days_back}back'
                        os.makedirs(self.saved_graph_dir, exist_ok=True)
                    elif self.mode == "sensitivity_analysis_days":
                        # No logic changes required here, this is a pretty self contained trial
                        self.saved_graph_dir = f'data/output/sensitivity_analysis/days/constructed_graphs/{self.dataset}_{self.common_suffix}_edgebank_{self.edgebank_style}_VectorType{encoder_config["use_test_style"] if encoder_config["use_predicted_vals"] else "TrueVals"}_len{toper_len}_days{days_back}back'
                        os.makedirs(self.saved_graph_dir, exist_ok=True)
                    ...
                    if ablation_mode == 9 and self.mode == "ablation":
                        self.old_vars = {
                                'graph_descriptions': self.graph_descriptions,
                                'thresholds': self.thresholds,
                                'target_graphs': self.target_graphs,
                        ...
                        self.graph_descriptions = base_toper
                        self.probabilities = base_probs
                        self.target_graphs = self.old_vars['target_graphs']
                        self.thresholds = self.old_vars['thresholds']
                    ...
                    # Helps experiments work clearly
                    if torch.cuda.is_available():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mode", type=str, default="default", choices=["default", "ablation", "sensitivity_analysis_len", "sensitivity_analysis_days"])
