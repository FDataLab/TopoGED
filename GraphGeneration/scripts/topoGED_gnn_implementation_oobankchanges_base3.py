...
# Base3 
from GraphGeneration.utils.Base3.Base3 import *
...
class Runner(object):
    def __init__(self):      
        ...
        self.saved_input = os.path.abspath(f'data/input/cached/{encoder_config["dataset"]}/saved_data_gnn_{self.model_type}{self.feature_type}_lr{encoder_config["training"]["lr"]}_{days_back_val}back_oobankchanges_base3_newnodestrategy{encoder_config['new_node_strategy']}')
        ...
        self.saved_graph_dir = f'data/output/constructed_graphs/{encoder_config["dataset"]}_{self.common_suffix}_base3'
        self.training_plots_path = f'GraphGeneration/output/results/training_plots/{encoder_config["dataset"]}/{encoder_config["encoder_model"]["nodeEmbeddingType"]}_{self.feature_type}_lr{encoder_config["training"]["lr"]}_{days_back_val}back_oobankchanges_base3'
    ...
    def build_accumulating_filtration_sequence_with_edgebank(self, current_target_snapshot):
        ...
        # Base3 Implementation
        # Inspired by https://github.com/ekmpa/Base3/blob/main/TGB-Base3.py but modified for discrete time snapshots
        
        # Init modules
        t_comem = tCoMem(time_window=len(prev_graphs), co_occurrence_weight=0.8)  # Default lambda
        method = 'multi_conf'  # Proven by authors to be strongest
        ...
        node_map = {}  # Maps node ids back to original
        reverse_map = {}
        curr_id = 0
        snapshot_dsts = []
        for t, graph in enumerate(prev_graphs):
            curr_dsts = []  # Used for PopTrack (we use a batch as a snapshot)
            
            for node in graph.nodes():
                if node not in node_map:
                    node_map[node] = curr_id
                    reverse_map[curr_id] = node
                    curr_id += 1
                    curr_nodes.add(node)
                    
                    
            for u, v in graph.edges():
                u_id = node_map[u] if node_map else u
                v_id = node_map[v] if node_map else v 
                curr_dsts.append(v_id)
                ...
                if not self.is_directed:
                    src_list.append(v_id)
                    dst_list.append(u_id)
                    ts_list.append(t)
                    curr_dsts.append(u_id)
                    
            # Update popularity scores
            snapshot_dsts.append(curr_dsts)
        
        
        poptrack = PopTrack(num_nodes=len(curr_nodes), decay=0.95)  # Default decay
        for dsts in snapshot_dsts:
            if dsts:
                poptrack.update_batch(dsts)
        src_list = np.array(src_list)
        dst_list = np.array(dst_list)
        ts_list = np.array(ts_list)
        
        # All edgebank style choices
        edgebank_memory = edge_bank_unlimited_memory(src_list, dst_list)
        # edgebank_memory = edge_bank_infin_freq(src_list, dst_list)
        # edgebank_memory = edge_bank_repetition_based_memory(src_list, dst_list)
        # edgebank_memory = time_window_edge_memory(src_list, dst_list, ts_list, start_time=0, end_time=self.days_back)
        # edgebank_memory = edge_bank_unlimited_memory(src_list, dst_list, ts_list, window_mode='fixed', memory_span=0.01)
        # edgebank_memory = edge_bank_unlimited_memory(src_list, dst_list, ts_list, window_mode='avg_reoccur', memory_span=0.01)

        # Update novel tcomem module
        for u, v, t in zip(src_list, dst_list, ts_list):
            t_comem.update(u, v, t)
        
        k_value = int(min(1000, max(10, len(curr_nodes) * 0.10)))  # Number of popular nodes, we will use 10%
        top_k_nodes = poptrack.predict_batch(k=k_value)
        all_ids = list(range(curr_id))
        
        if self.is_directed:
            candidate_edges = list(itertools.permutations(all_ids, 2))
        else:
            candidate_edges = list(itertools.combinations(all_ids, 2))
        
        scored_candidates = []
        next_t = len(prev_graphs)
        
        for u, v in candidate_edges:
            score = full_interpolated_score(
                u, v, next_t,
                edgebank_memory, poptrack, top_k_nodes, t_comem,
                method='multi_conf'
            )
            # Optimization: Only store non-zero scores to save memory
            if score > 0:
        ...
        curr_edges_oobank = []
        target_count = self.current_target_count['o-o-bank']
        
        for (u_id, v_id), score in scored_candidates[:target_count]:
            u_orig = reverse_map[u_id]
            v_orig = reverse_map[v_id]
            curr_edges_oobank.append((u_orig, v_orig))
            
        curr_old_nodes = set()      
        for u, v in curr_edges_oobank:
            curr_old_nodes.add(u)
            curr_old_nodes.add(v)
                    
        
        if len(curr_old_nodes) < target_nodes:
            remaining_candidates = list(curr_nodes - curr_old_nodes)
        ...
        for ablation_mode in ablation_modes:
            for toper_len in sensitivity_params:
                ...
                if ablation_mode > 0:
                    ...
                    self.saved_graph_dir = f'data/output/ablation/constructed_graphs/{encoder_config["dataset"]}_{self.common_suffix}_base3_ablation{ablation_mode}'
                    ...
                    self.target_graphs, _ = modifyGraphIds(self.target_graphs, self.thresholds, 10000)
                    self.graph_descriptions = [[(lst[i], lst[i+1]) for i in range(0, len(lst), 2)] for lst in self.graph_descriptions]
                    self.saved_graph_dir = f'data/output/sensitivity_analysis/constructed_graphs/{encoder_config["dataset"]}_{self.common_suffix}_base3_len{toper_len}'
                    os.makedirs(self.saved_graph_dir, exist_ok=True)
