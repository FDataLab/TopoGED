...
class Runner(object):
    ...
    def train_multi_head(self, edge_type, X_train, y_train, X_val=None, y_val=None, lr=1e-3, epochs=250, batch_size=64, top_k=0, seed=1024):
        """
    ...
    def train_models(self, lr=0.001, seed=1024):
        """
        ...
        training_sorted_samples, training_new_edges_count = generate_training_data_cached(training_graphs=self.training_graphs, old_nodes=old_nodes, 
                                                all_edgebanks=self.all_edgebanks, MAX_SAMPLES=MAX_SAMPLES, dataset=args.dataset, seed=global_seed)
        ...
        validation_sorted_samples, training_new_edges_count = generate_validation_data_cached(training_graphs=self.validation_graphs, old_training_nodes=old_training_nodes, 
                                                all_edgebanks=self.all_edgebanks[self.train_end], MAX_SAMPLES=MAX_SAMPLES, dataset=args.dataset, seed=global_seed, type_data="validation")
        # Prepare test data
        # We pass all_edgebanks of the training snapshots edgebanks
        test_sorted_samples, training_new_edges_count = generate_validation_data_cached(training_graphs=self.test_graphs, old_training_nodes=old_training_nodes, 
                                                all_edgebanks=self.all_edgebanks[self.train_end], MAX_SAMPLES=MAX_SAMPLES, dataset=args.dataset, seed=global_seed, type_data="test")
        ...
        for flag in ['o-o-bank', 'o-o-nobank']:
            ...
            self.link_prediction_decoder = self.train_multi_head(edge_type=flag, X_train=X_train, y_train=y_train, X_val=X_val, y_val=y_val, 
                                                                 lr=lr, epochs=500, batch_size=64, top_k=training_new_edges_count[flag], seed=seed)
    ...
    def run(self): 
        ...
        self.link_prediction_decoder = self.train_models(lr=args.lr, seed=global_seed)
