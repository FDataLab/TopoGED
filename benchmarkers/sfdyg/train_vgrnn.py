...
class VgrnnLinkPrediction(LinkPrediction):
    ...
    def train(self, args, model, dataset, split):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            # 1. Train strictly on the Train split
            ...
            pbar.set_description(f"E({epoch:02d}) train_loss={train_loss:.4f}, time={t1-t0:.2f}s, best_loss={best_train_loss:.4f} | wandering={wandering}")
            pbar.update(1)

            if wandering > args.patience:
                print("\nStopping early: Training loss has plateaued.")
                break
