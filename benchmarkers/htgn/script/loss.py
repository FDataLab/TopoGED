...
class ReconLoss(nn.Module):
    ...
    # ADDED BY US FOR GRAPH CONSTRUCTION
    def get_probs(self, z):
        """
        Calculates the full N x N probability matrix.
        Essential for threshold optimization and NetworkX graph construction.
        """
        # 1. Compute pairwise distances in hyperbolic space [cite: 892]
        # manifold.sqdist computes the squared distance in the Poincaré ball
        num_nodes = z.size(0)
        
        # To avoid memory explosion on large graphs, we compute pairwise distances
        # If nodes are too many (>5000), you may need a chunked approach.
        # This mirrors the logic in HTGN Eq. 12
        dist = self.manifold.sqdist(z.unsqueeze(1), z.unsqueeze(0), c=1.0)
        dist = torch.sqrt(torch.clamp(dist, min=EPS))
        
        # 2. Decode to probabilities 
